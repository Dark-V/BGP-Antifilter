...
- `BGP_ANTIFILTER_VERSION` - тег локального Docker-образа, по умолчанию `0.4.4`.
- `MT_AS` - AS MikroTik.
...
- `Списки` - редактирование `lists.txt`, `include-asns.txt`, `include-countries.txt`, `include-domains.txt`, `exclude-domains.txt`; URL списков доменов управляются отдельным блоком внутри вкладки `Include domains`.
...
Для URL со списками именно доменов используйте блок `Источники списков доменов` внутри вкладки `Include domains` (технически он хранится в `generated/config/domain-list-urls.txt`). Формат содержимого источника простой: один домен на строку; пустые строки и строки, начинающиеся с `#`, игнорируются. Домены нормализуются, дубли удаляются, затем каждый домен резолвится в IPv4 и найденные адреса добавляются как `/32`.
...
В `generated/config/domain-list-urls.txt` источник можно записать с именем:

```text
YouTube | https://example.com/list.txt
```

Формат: `Имя | URL`. Имя необязательно: строка только с URL тоже поддерживается, а админка тогда автоматически показывает имя по хосту/имени файла. Строки с `#` остаются обычными комментариями и не используются как подписи источников.
